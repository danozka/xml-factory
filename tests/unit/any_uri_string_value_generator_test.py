...
def test_random_uri_with_exact_length_restriction(generator: AnyUriStringValueGenerator) -> None:
    restriction: Restriction = Restriction(name='test', base_type=BaseType(BaseType.any_uri), length=15)
...
def test_random_uri_with_min_length_restriction(generator: AnyUriStringValueGenerator) -> None:
    min_length: int = 20
    restriction: Restriction = Restriction(name='test', base_type=BaseType(BaseType.any_uri), min_length=min_length)
    uri: str = generator.generate_random_any_uri_string_value(restriction)
...
def test_random_uri_with_max_length_restriction(generator: AnyUriStringValueGenerator) -> None:
    max_length: int = 25
    restriction: Restriction = Restriction(name='test', base_type=BaseType(BaseType.any_uri), max_length=max_length)
    uri: str = generator.generate_random_any_uri_string_value(restriction)
...
def test_default_length_boundaries_when_not_specified(generator):
    restriction: Restriction = Restriction(name='test', base_type=BaseType(BaseType.any_uri))
    uri: str = generator.generate_random_any_uri_string_value(restriction)
