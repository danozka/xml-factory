...
class RestrictionMinValueGenerator(BaseRestrictionValueGenerator, IRestrictionValueGenerator):
    def generate_restriction_value(self, restriction: Restriction) -> str:
        if restriction.base_type == BaseType.any_uri:
            ...
            return self._date_string_value_generator.generate_min_date_string_value(restriction)
        elif restriction.base_type == BaseType.date_time:
            return self._date_time_string_value_generator.generate_min_date_time_string_value(restriction)
        elif restriction.base_type == BaseType.decimal or restriction.base_type == BaseType.float:
            return self._decimal_string_value_generator.generate_min_decimal_string_value(restriction)
        elif restriction.base_type == BaseType.gregorian_day:
            return self._gregorian_day_string_value_generator.generate_min_gregorian_day_string_value(restriction)
            ...
            return self._gregorian_month_day_string_value_generator.generate_min_gregorian_month_day_string_value(
                restriction
            )
        elif restriction.base_type == BaseType.gregorian_year:
            return self._gregorian_year_string_value_generator.generate_min_gregorian_year_string_value(restriction)
        elif restriction.base_type == BaseType.gregorian_year_month:
            return self._gregorian_year_month_string_value_generator.generate_min_gregorian_year_month_string_value(
                restriction
            )
            ...
            return self._integer_string_value_generator.generate_min_integer_string_value(restriction)
        elif restriction.base_type == BaseType.string:
            if restriction.min_length is not None:
                return self._string_value_generator.generate_specific_length_string_value(restriction.min_length)
            else:
                return self._string_value_generator.generate_random_string_value(restriction)
        elif restriction.base_type == BaseType.time:
            return self._time_string_value_generator.generate_min_time_string_value(restriction)
        else:
